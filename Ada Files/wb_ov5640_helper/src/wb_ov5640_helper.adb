...
with Wb_Ov5640_Address_Map;     use Wb_Ov5640_Address_Map;
...
package body Wb_Ov5640_Helper is
   ...
   --Method to write to an OV5640 register. Intentionally the same method signature as the application datasheet
   procedure write_i2c (OV5640_reg_addres : Word; OV5640_reg_data : Word) is
      Manual_SCCB_Status : Word;
   begin
      if (Is_Camera_Programmed) then
         Write_Reg (SCCB_REG_ADDR_Addr, OV5640_reg_addres);
         Write_Reg (SCCB_REG_DATA_Addr, OV5640_reg_data);
         Write_Reg
           (SCCB_USER_CONTROL_Addr, Word (1)); --Start manual programming
         loop
            Manual_SCCB_Status := Read_Reg (SCCB_USER_STATUS_Addr);
            exit when ((Manual_SCCB_Status and Word (1)) = 0);
         end loop;
         Write_Reg (SCCB_USER_CONTROL_Addr, Word (0));
      end if;
   end write_i2c;
